...
namespace SteffenTools.Extensions
{
    public static class AdditionalGizmos
    {
        ...
        public static void DrawArrowLine(Vector3 origin, Vector3 destination, float arrowSpokeSize = .4f)
        {
            Gizmos.DrawLine(origin, destination);
            //var arrowSpoke = new Vector3(Mathf.Sin((float)i / 4 * 360 * Mathf.Deg2Rad), Mathf.Cos((float)i / 4 * 360 * Mathf.Deg2Rad), 0);

            var arrowSpoke = (origin - destination);
            arrowSpoke = arrowSpoke.normalized;

            arrowSpoke = Quaternion.Euler(0, 0, 30) * arrowSpoke;
            arrowSpoke.Normalize();
            Gizmos.DrawRay(destination, arrowSpoke * arrowSpokeSize);
            arrowSpoke = Quaternion.Euler(0, 0, -60) * arrowSpoke;
            arrowSpoke = arrowSpoke.normalized;
            Gizmos.DrawRay(destination, arrowSpoke * arrowSpokeSize);
        }
    }
